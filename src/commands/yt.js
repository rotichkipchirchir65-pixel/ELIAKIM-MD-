...
export async function ytCmd(sock, msg, { jid, args, command }) {
  ...
  try {
    ...
    stream.on("data", (c) => { total += c.length; chunks.push(c); if (total > 50e6) stream.destroy(new Error("Too large")); });
    await new Promise((res, rej) => { stream.on("end", res); stream.on("error", rej); });
    const buffer = Buffer.concat(chunks);

    if (isVideo) await sock.sendMessage(jid, { video: buffer, caption: `🎬 *${title}*` }, { quoted: msg });
    else { await sock.sendMessage(jid, { audio: buffer, mimetype: "audio/mp4" }, { quoted: msg }); await sock.sendMessage(jid, { text: `🎵 *${title}*` }); }
  } catch (err) {
